...
const layers: Layer[] = [
    {
        ...
        color: 'var(--color-grey-olive-400)',
        components: [
    ...
    },
    {
        ...
        color: 'var(--color-vintage-grape-400)',
        components: [
    ...
    },
    {
        ...
        color: 'var(--color-stone-brown-400)',
        components: [
    ...
    },
    {
        ...
        color: 'var(--color-khaki-beige-400)',
        components: [
...
const Architecture = () => {
    ...
    return (
        <section id="architecture" className="py-32 relative overflow-hidden bg-transparent">
            {/* Background accent */}
            <div
            ...
            <div className="max-w-7xl mx-auto px-6 relative z-10">
                ...
                <div className="max-w-3xl mb-20">
                    <h2 className="text-5xl md:text-6xl lg:text-7xl font-bold font-serif leading-[1.1] mb-6">
                        Technical <span className="gradient-text text-accent-grape">Architecture</span>
                    </h2>
                        ...
                        {layers.map((layer) => (
                            <button
                                ...
                                className={`w-full text-left p-6 rounded-2xl border transition-all duration-500 group ${activeLayer === layer.id
                                    ? 'bg-white/[0.03] border-accent-grape/30 shadow-[0_0_40px_rgba(119,101,154,0.1)]'
                                    : 'bg-transparent border-custom hover:border-text-muted/10 hover:bg-white/[0.01]'
                                    }`}
                            >
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        ...
                                        />
                                        <span className={`text-xl font-semibold transition-colors duration-300 ${activeLayer === layer.id ? 'text-text-primary' : 'text-text-secondary group-hover:text-text-primary'
                                            }`}>
                                            {layer.title}
                                        </span>
                                    </div>
                                    <svg
                                        ...
                                        stroke="currentColor"
                                        strokeWidth="1.5"
                                        className={`transition-all duration-500 ${activeLayer === layer.id ? 'text-accent-grape rotate-0' : 'text-text-muted -rotate-90'
                                            }`}
                                    >
                                        <path d="M6 9l6 6 6-6" />
                                    </svg>
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="grid gap-4">
                                {activeData?.components.map((comp, idx) => (
                                    <div
                                        key={idx}
                                        className="bg-white/[0.02] hover:bg-white/[0.05] border border-custom hover:border-text-muted/10 rounded-xl p-5 transition-all duration-300 group cursor-default"
                                        style={{ animationDelay: `${idx * 100}ms` }}
                                    >
                                        <div className="flex items-center justify-between">
                                            <div>
                                                <div className="font-semibold text-text-primary mb-1 group-hover:text-accent-grape transition-colors">
                                                    {comp.name}
                                                </div>
                                            ...
                                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-text-muted group-hover:text-accent-grape group-hover:translate-x-1 transition-all">
                            ...
                            {/* Code snippet */}
                            <div className="mt-8 bg-primary rounded-xl overflow-hidden border border-custom">
                                <div className="flex items-center justify-between px-4 py-3 border-b border-custom">
                                    ...
                                    <span className="text-xs text-text-muted font-mono">solstice.rs</span>
                                </div>
