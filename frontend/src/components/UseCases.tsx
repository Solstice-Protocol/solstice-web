        ...
        <section ref={sectionRef} className="py-24 px-6 bg-[var(--color-bg-primary)] border-t border-[var(--color-border)]">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-[var(--color-border)] border border-[var(--color-border)]">
                    {cases.map((c, i) => (
                        <div key={i} className="use-case-card opacity-0 bg-[var(--color-bg-primary)] p-8 hover:bg-[var(--color-bg-secondary)] transition-colors duration-300">
                            <h3 className="text-2xl mb-2 font-normal text-[var(--color-text-primary)] font-serif italic">
                                {c.title}
                            </h3>
                            <p className="text-[var(--color-text-secondary)] font-serif">
                                {c.desc}
                            </p>
                        </div>
                    ))}
